    ...
    if request.method == 'POST':
        ...
        if password1 == password2:
            ...
            with transaction.atomic():
                existing_user_username = CustomUser.objects.filter(username=username).first()
                ...
                user = CustomUser.objects.create_user(username=username, password=password1, email=email, phone=phone, is_verified=False)
                user.save()
...
from django.urls import reverse
from django.utils import timezone
from .models import CustomUser, Storedotps
...
def password_update(request):
    if request.method == 'POST':
        ...
        if password == password1:
            ...
            try:
                CustomUser.objects.filter(email=request.session.get('user_email')).update(password=password)
    ...
    if request.method == 'POST':
        ...
        if password == password1:
            ...
            try:
                ...
                messages.info(request, "something Went wrong.....")
                return render(request, "main/somethingwrong.html")
        else:
            messages.info(request,'Password not Matched')
            return redirect('login:profile_password_update')
...
def resend_otp(request):
    ...
    if user_email and User.objects.filter(email=user_email).exists():
        ...
        return render(request, "main/forgot_password.html", {'email': user_email})

    return render(request, "main/forgot_password.html")

from .models import CustomUser  # Import your custom Profile model

def profile_resend_otp(request):
    user_email = request.session.get('user_email', None)

    if user_email and CustomUser.objects.filter(email=user_email).exists():
        user_profile = CustomUser.objects.get(email=user_email)  # Use your custom Profile model
        OTP = random.randint(100000, 999999)

        subject = 'OTP for Forgot Password'
        message = f'Hi {user_profile.username}, Your Forgot Password OTP is {OTP} Valid For 5 Minutes. Do Not Share. If you did not request for OTP, please ignore this message.'
...
def check_currentpassword(request):
    ...
    if CustomUser.objects.filter(password=password,is_verified=True).exists():
